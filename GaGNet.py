...
class GlanceGazeModule(nn.Module):
    def __init__(self,
        ...
        self.glance_block = GlanceBlock(kd1, cd1, d_feat, p, dilas, fft_num, is_causal, acti_type)
        self.gaze_block = GazeBlock(kd1, cd1, d_feat, p, dilas, fft_num, is_causal, is_squeezed)
...
class GlanceBlock(nn.Module):
    def __init__(self,
        ...
        tcn_g_list = []
        for _ in range(p):
            tcn_g_list.append(SqueezedTCNGroup(kd1, cd1, d_feat, dilas, is_causal))
...
class GazeBlock(nn.Module):
    def __init__(self,
        ...
        if not is_squeezed:
            ...
            for _ in range(p):
                tcn_r_list.append(SqueezedTCNGroup(kd1, cd1, d_feat, dilas, is_causal))
                tcn_i_list.append(SqueezedTCNGroup(kd1, cd1, d_feat, dilas, is_causal))
            self.tcm_r, self.tcm_i = nn.Sequential(*tcn_r_list), nn.Sequential(*tcn_i_list)
        else:
            tcn_ri_list = []
            for _ in range(p):
                tcn_ri_list.append(SqueezedTCNGroup(kd1, cd1, d_feat, dilas, is_causal))
...
class SqueezedTCNGroup(nn.Module):
    def __init__(self,
        ...
        for i in range(len(dilas)):
            tcn_list.append(SqueezedTCM(kd1, cd1, d_feat, dilas[i], is_causal))
...
class SqueezedTCM(nn.Module):
    def __init__(self,
        ...
        self.d_conv = nn.Sequential(
            nn.PReLU(cd1),
            nn.InstanceNorm1d(cd1, affine=True, track_running_stats=True),
...
class U2Net_Encoder(nn.Module):
    def __init__(self,
        ...
        meta_unet = []
        meta_unet.append(
            En_unet_module(cin, c, k_beg, k2, intra_connect, scale=4))
        meta_unet.append(
            En_unet_module(c, c, k1, k2, intra_connect, scale=3))
        meta_unet.append(
            En_unet_module(c, c, k1, k2, intra_connect, scale=2))
        meta_unet.append(
            En_unet_module(c, c, k1, k2, intra_connect, scale=1))
        self.meta_unet_list = nn.ModuleList(meta_unet)
        self.last_conv = nn.Sequential(
            GateConv2d(c, c_end, k1, (1,2)),
            nn.InstanceNorm2d(c_end, affine=True, track_running_stats=True),
            nn.PReLU(c_end)
        )
    def forward(self, x: Tensor) -> Tensor:
...
class UNet_Encoder(nn.Module):
    def __init__(self,
        ...
        unet.append(nn.Sequential(
            ...
            nn.InstanceNorm2d(c, affine=True, track_running_stats=True),
            nn.PReLU(c)))
        unet.append(nn.Sequential(
...
class En_unet_module(nn.Module):
    def __init__(self,
        ...
        in_conv_list.append(nn.InstanceNorm2d(cout, affine=True, track_running_stats=True))
...
class Deconv2dunit(nn.Module):
    def __init__(self,
        ...
        deconv_list.append(nn.InstanceNorm2d(c, affine=True, track_running_stats=True))
